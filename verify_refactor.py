...
def test_dynamic_rating():
    print("\nTesting Dynamic Rating...")
    # 1. Create a movie
    movie_payload = {"title": "Rating Test Movie", "year": 2023, "genre": "drama"}
    response = requests.post(f"{BASE_URL}/movies", json=movie_payload)
    ...
    # 2. Check initial rating (should be 0)
    response = requests.get(f"{BASE_URL}/movies/{movie_id}/rating")
    assert response.status_code == 200
    assert response.json() == 0.0
    print("Initial rating is 0.0")

    # 3. Create a user
    user_payload = {"username": "rating_user", "email": "rating@example.com", "password": "password"}
    response = requests.post(f"{BASE_URL}/users", json=user_payload)
    # Handle user exists
    if response.status_code == 409:
         # login to get id if needed, but for rating we need user_id. 
         # Let's just assume user 1 exists or create a unique one
         pass
    
    # For simplicity, let's use existing users or create a unique one
    import time
    unique_user = f"user_{int(time.time())}"
    user_payload = {"username": unique_user, "email": f"{unique_user}@example.com", "password": "password"}
    response = requests.post(f"{BASE_URL}/users", json=user_payload)
    user_id = response.json()["id"]

    # 4. Add a rating
    rating_payload = {"user_id": user_id, "movie_id": movie_id, "rating": 5.0}
    response = requests.post(f"{BASE_URL}/user-ratings", json=rating_payload)
    assert response.status_code == 200
    print("Added rating 5.0")

    # 5. Check rating again (should be 5.0)
    response = requests.get(f"{BASE_URL}/movies/{movie_id}/rating")
    assert response.status_code == 200
    assert response.json() == 5.0
    print("Rating updated to 5.0")
    
    # 6. Add another rating
    unique_user_2 = f"user_2_{int(time.time())}"
    user_payload_2 = {"username": unique_user_2, "email": f"{unique_user_2}@example.com", "password": "password"}
    response = requests.post(f"{BASE_URL}/users", json=user_payload_2)
    user_id_2 = response.json()["id"]
    
    rating_payload_2 = {"user_id": user_id_2, "movie_id": movie_id, "rating": 3.0}
    response = requests.post(f"{BASE_URL}/user-ratings", json=rating_payload_2)
    assert response.status_code == 200
    print("Added rating 3.0")

    # 7. Check rating again (should be 4.0)
    response = requests.get(f"{BASE_URL}/movies/{movie_id}/rating")
    assert response.status_code == 200
    assert response.json() == 4.0
    print("Rating updated to 4.0")

    print("Dynamic rating test passed!")
